...
import { getRealStateReviews } from '../../utils/action'

moment.locale("es")

export default function ListReview({ navigation, id}) {
    ...
    useFocusEffect(
        useCallback(() => {
            (async() => {
                const response = await getRealStateReviews(id)
        ...
        <View>
            {
                userLogged ? (
                    <Button
                        ...
                        onPress={() => navigation.navigate("add-review-state", { id })}
                        icon={{
...
function Review ({ reviewDocument }) {
    ...
    const { review, createAt, avatarUser, rating } = reviewDocument 
